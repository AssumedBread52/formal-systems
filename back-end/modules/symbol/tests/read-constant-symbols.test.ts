...
describe('Read Constant Symbols', (): void => {
  ...
  it('fails without a token', async (): Promise<void> => {
    await testMissingToken(app, 'get', '/system/1/symbol/constant');
  });

  it('fails with an expired token', async (): Promise<void> => {
    await testExpiredToken(app, 'get', '/system/1/symbol/constant');
  });

  it('fails with an invalid token', async (): Promise<void> => {
    await testInvalidToken(app, 'get', '/system/1/symbol/constant');
  });
