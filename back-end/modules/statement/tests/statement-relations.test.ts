...
describe('Statement Relations', (): void => {
  ...
  it('POST /graphql query statement', async (): Promise<void> => {
    ...
    const response = await request(app.getHttpServer()).post('/graphql').send({
      query: 'query ($systemId: String!, $statementId: String!) { statement(systemId: $systemId, statementId: $statementId) { id systemId assertionExpressionId name description system { id ownerUserId name description } } }',
      variables: {
        systemId,
        statementId
      }
