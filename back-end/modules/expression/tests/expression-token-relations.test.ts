...
describe('Expression Token Relations', (): void => {
  ...
  it('POST /graphql query expression', async (): Promise<void> => {
    ...
    const response = await request(app.getHttpServer()).post('/graphql').send({
      query: 'query ($systemId: String!, $expressionId: String!) { expression(systemId: $systemId, expressionId: $expressionId) { id systemId canonical expressionTokens { systemId symbolId expressionId position system { id ownerUserId name description } symbol { id systemId name description type content } expression { id systemId canonical } } } }',
      variables: {
        systemId,
        expressionId
      }
