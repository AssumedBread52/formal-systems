...
export class ExpressionController {
  ...
  public deleteExpression(@SessionUser('id') sessionUserId: string, @Param('systemId') systemId: string, @Param('expressionId') expressionId: string): Promise<ExpressionEntity> {
    return this.expressionWriteService.delete(sessionUserId, systemId, expressionId);
  }

  @Get()
  @UseInterceptors(ClassSerializerInterceptor)
  public getExpressions(@Param('systemId') systemId: string, @Query(new ValidationPipe({ transform: true })) searchExpressionsPayload: SearchExpressionsPayload): Promise<PaginatedExpressionsPayload> {
    return this.expressionReadService.searchExpressions(systemId, searchExpressionsPayload);
  }

  @Get(':expressionId')
  @UseInterceptors(ClassSerializerInterceptor)
  public getById(@Param('systemId') systemId: string, @Param('expressionId') expressionId: string): Promise<ExpressionEntity> {
  ...
  @UseGuards(JwtGuard)
  @UseInterceptors(ClassSerializerInterceptor)
  public postExpression(@SessionUser('id') sessionUserId: string, @Param('systemId') systemId: string, @Body(new ValidationPipe({ transform: true })) newExpressionPayload: NewExpressionPayload): Promise<ExpressionEntity> {
